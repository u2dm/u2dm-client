...
use slint::{
    Image, Model, Rgba8Pixel, SharedPixelBuffer, SharedString, Timer, TimerMode, VecModel,
};
...
fn rgba_to_image(rgba: &image::RgbaImage) -> Image {
    ...
    let pixels = SharedPixelBuffer::<Rgba8Pixel>::clone_from_slice(rgba.as_raw(), width, height);
...
fn frame_to_image(frame: image::Frame) -> Image {
    rgba_to_image(&frame.into_buffer())
}
...
fn decode_animation(path: &Path, budget: usize) -> Option<Animation> {
    ...
    for frame in frames_of(path)? {
        ...
        bytes =
            bytes.saturating_add(image.size().width as usize * image.size().height as usize * 4);
        if bytes > budget {
            tracing::debug!(
...
fn animation_cached(path: &Path) -> Option<Rc<Animation>> {
    ...
    let budget = ANIMATION_MEMORY_BUDGET.saturating_sub(cached_animation_bytes());
...
pub fn load_thumbnail(path: &Path, event_id: &str) -> Option<Image> {
    ...
    animation
        .frame(frame)
        .cloned()
        .or_else(|| load_image_cached(path))
}
