...
pub const CATALOG: Scenarios = Scenarios {
    ...
    flags: &[
        ...
        Flag {
            value: "all-unread",
            effect: "every loaded message is unread, so the anchor is the top row",
        ...
        Flag {
            value: "unread-unresolved",
            effect: "the read position cannot be placed in the loaded window, so opening the room sends no receipt",
            note: "excluded from `all`; the evidence is the `withholding the read receipt until the unread boundary resolves` line",
        },
        Flag {
            value: "jump-far",
...
pub struct Scenario {
    ...
    pub unread_boundary_is_unresolved: bool,
...
fn from_env() -> Scenario {
    ...
    tracing::info!(
        ...
        unread_boundary_is_unresolved = scenario.unread_boundary_is_unresolved,
...
fn apply(scenario: &mut Scenario, flag: &str) {
    match flag {
        ...
        "unread-unresolved" => scenario.unread_boundary_is_unresolved = true,
        ...
        "all" => {
            *scenario = Scenario {
                ...
                unread_boundary_is_unresolved: false,
